# Vendor Portal（VP）供应商自助结算 规格文档 V6
...
- `src/docs/prds/S44 VP Billing/VP billing V9.md`
...
### My Statements 对外状态简化（StatementList）

VP 端向供应商展示更简洁的状态标识，隐藏内部比对/支付流程细节：

| 内部状态 | VP 对外标签 | 含义 |
|---|---|---|
| `Awaiting Comparison` | **Processing** | TMS 正在对账，无需供应商操作 |
| `Pending Payment` | **Processing** | 已通过对账，正在打款 |
| `Awaiting Re-bill` | **Action Required** | 对账异常，需供应商重新提单 |
| `Paid` | **Paid** | 已完成打款 |

### My Statements Release Proof（StatementList）

- `Paid` 状态行新增 **Release Proof** 列，展示财务打款凭证文件链接（点击可下载）
- 无凭证时显示 `—`
